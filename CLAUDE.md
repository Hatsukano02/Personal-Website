...
- 每个技术决策必须经过完整的链路分析
- 必须识别和解决根本原因，不能治标不治本
- 复杂问题必须使用 Sequential Thinking MCP 进行结构化思考
- 所有修改必须考虑对整个系统的影响

### 🧠 Sequential Thinking MCP 强制使用场景

**以下情况必须使用 Sequential Thinking MCP：**
...
4. **问题解决不明确时**
   - 当第一反应是"简单修改"时，必须暂停使用 Sequential Thinking
   - 当遇到之前未处理过的问题类型时
   - 当解决方案可能影响多个功能时

**使用方法：**
```
使用 Sequential Thinking MCP 分析这个问题：[具体问题描述]
需要考虑的方面：[列出关键考虑点]
```
...
#### 2. 完整技术链路分析
- 配置文件检查：相关的所有配置文件
- 代码实现验证：组件中实际使用的API调用
- 依赖关系追踪：导入导出关系、版本兼容性
- 构建产物检查：确保配置正确编译到最终产物
...
本项目配置了六个 MCP (Model Context Protocol) 服务器，每个都有特定的使用场景：

#### 1. **Local Filesystem (mcp__filesystem__)** 
**❌ 不应在本项目中使用**
- **用途**: 访问外部目录的文件系统操作
- **误用**: 在当前项目中读写文件时无需使用此 MCP
- **正确做法**: 直接使用 Read、Write、Edit 等内置工具

#### 2. **GitHub (mcp__github__)** 
**✅ 跨仓库操作时使用**
- **用途**: 与 GitHub API 交互，管理仓库、PR、Issue
- **使用场景**:
  - 创建/更新远程仓库文件
  - 管理 Pull Requests 和 Issues  
  - 分支管理和发布操作
  - 搜索其他 GitHub 仓库代码
- **工具**: `create_repository`, `create_pull_request`, `search_code` 等

#### 3. **Omnisearch (mcp__omnisearch__)** 
**✅ 网络搜索时使用**
- **用途**: 多引擎网络搜索和内容提取
- **使用场景**:
  - 搜索最新技术文档
  - 获取实时信息和趋势
  - 网页内容提取和分析
- **工具**: `tavily_search`, `tavily_extract_process`
...
#### 6. **Playwright (mcp__playwright__)** 
**✅ 网页自动化和测试时使用**
- **用途**: 浏览器自动化，无需视觉模型的网页交互
- **使用场景**:
  - E2E 测试自动化
  - 网页抓取和数据提取
  - UI 交互测试
  - 网页截图和 PDF 生成
- **特点**: 基于可访问性树而非像素，更可靠和快速
- **工具**: `browser_navigate`, `browser_click`, `browser_snapshot` 等

#### 使用原则
1. **本地项目文件**: 直接使用内置的 Read、Write、Edit 工具
2. **外部网络资源**: 根据需求选择对应的 MCP 服务器
3. **复杂任务**: 优先使用 Sequential Thinking 进行结构化分析
4. **最新文档**: 需要时明确指定使用 Context7
